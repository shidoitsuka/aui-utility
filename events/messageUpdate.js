...
module.exports = {
  ...
  async execute(oldMessage, newMessage) {
    ...
    }:${time.getMinutes() < 10 ? "0" + time.getMinutes() : time.getMinutes()}] ${day[time.getDay()]}, ${
    ...
    const embed = new MessageEmbed()
      ...
      .addFields(
        { name: "**Original Message**", value: oldMsg.content.length > 1020 ? `${oldMsg.content.substring(0, 1020)}...` : oldMsg.content },
        { name: "**Edited Message**", value: newMessage.content.length > 1020 ? `${newMessage.content.substring(0, 1020)}...` : newMessage.content },
        ...
        { name: "**Channel ID**", value: `\`${oldMsg.channel.id}\``, inline: true },
    ...
    bot.guilds.cache.get(bot.config.logGuildId).channels.cache.get(bot.db.get("messageUpdateChannel")).send({ embeds: [embed] }).catch(err => console.error("[MESSAGE UPDATE]", err));
